...
def get_rich():
    ...
    # 🚀 사장님 맞춤 타겟 앱 (옥션, 이후동행, 키워드 등 잡상인 완벽 차단)
                    ...
                    if len(txt) > 5 and href:
                        ...
                        if any(k in txt for k in ["퀴즈", "정답", "쏠", "하나", "원큐", "OX", "챌린지"]):
                            try:
                                ...
                                # 🚀 [핵심수정] 본문의 모든 태그를 제거하고 텍스트만 뽑아 '한 줄'로 펴기
                                p_soup = BeautifulSoup(p_res.text, 'html.parser')
                                for s in p_soup(['script', 'style', 'img', 'iframe']): s.decompose()
                                body_clean = re.sub(r'\s+', ' ', p_soup.get_text())
                                
                                # 🚀 [필살기] 정답 단어 뒤에 뭐가 있든 무시하고 80자 이내의 첫 알맹이 낚기
                        ...
                        # 🚀 제목에 퀴즈 단어 없으면 info는 "" 상태 -> [확인필요] 없이 제목만 깔끔!
                        clean_t = txt.split('\n')[0][:25]
