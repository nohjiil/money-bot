...
def get_rich():
                    ...
                    if len(txt) > 5 and href:
                        ...
                        if any(k in txt for k in ["퀴즈", "정답", "쏠", "하나", "원큐", "OX", "챌린지"]):
                            try:
                                ...
                                # 🚀 [수사 범위 대폭 확대] 정답 단어 이후 60자까지 뒤져서 OX나 단어를 찾습니다.
                                # '정답 :      X' 처럼 공백이 많아도 낚아옵니다.
                                match = re.search(r'(정답|답|정답은|답은).{0,60}?\s*[:=]?\s*([^\r\n\t\s,.<>]{1,12})', body, re.DOTALL)
                                if not match:
                                    match = re.search(r'\((\w{1,12})\)', body)

                                if match:
                                    ans_val = (match.group(2) if len(match.groups()) > 1 else match.group(1)).strip()
                                    # OX는 한 글자도 오케이! 나머지는 금지어 필터링
    ...
    try:
        ...
        requests.put(url, json={"message": "fix: expand search range for OX", "content": content, "sha": sha} if sha else {"message": "init", "content": content}, headers=h)
    except: pass
